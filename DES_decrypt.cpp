#include <bits/stdc++.h>
using namespace std;

string permutation(string str, int *arr, int n);
string decryption(string plainText, vector<string> round_key_binary, vector<string> round_key_array);

 // S-box Table
int SBox[8][4][16] = {
    {14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7,
     0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8,
     4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0,
     15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13},
    {15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10,
     3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5,
     0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15,
     13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9},
    {10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8,
     13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1,
     13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7,
     1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12},
    {7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15,
     13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9,
     10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4,
     3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14},
    {2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9,
     14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6,
     4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14,
     11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3},
    {12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11,
     10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8,
     9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6,
     4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13},
    {4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1,
     13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6,
     1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2,
     6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12},
    {13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7,
     1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2,
     7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8,
     2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11}
};

// Straight Permutation Table
int per[32] = {16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23,
    26, 5, 18, 31, 10, 2, 8, 24, 14, 32, 27,
    3, 9, 19, 13, 30, 6, 22, 11, 4, 25
};
    
int shift_table[16] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};

string Bin_To_Hex(string SBox){
    
    unordered_map<string, string> hexMap;
    hexMap["0000"] = "0";
    hexMap["0001"] = "1";
    hexMap["0010"] = "2";
    hexMap["0011"] = "3";
    hexMap["0100"] = "4";
    hexMap["0101"] = "5";
    hexMap["0110"] = "6";
    hexMap["0111"] = "7";
    hexMap["1000"] = "8";
    hexMap["1001"] = "9";
    hexMap["1010"] = "A";
    hexMap["1011"] = "B";
    hexMap["1100"] = "C";
    hexMap["1101"] = "D";
    hexMap["1110"] = "E";
    hexMap["1111"] = "F";
    string hex = "";
    for (int i = 0; i < SBox.length(); i += 4){
        string hex_part = "";
        hex_part += SBox[i];
        hex_part += SBox[i + 1];
        hex_part += SBox[i + 2];
        hex_part += SBox[i + 3];
        hex += hexMap[hex_part];
    }
    return hex;
}

string Hex_To_Bin(string SBox)
{
    // hexadecimal to binary conversion
    unordered_map<char, string> hexMap;
    hexMap['0'] = "0000";
    hexMap['1'] = "0001";
    hexMap['2'] = "0010";
    hexMap['3'] = "0011";
    hexMap['4'] = "0100";
    hexMap['5'] = "0101";
    hexMap['6'] = "0110";
    hexMap['7'] = "0111";
    hexMap['8'] = "1000";
    hexMap['9'] = "1001";
    hexMap['A'] = "1010";
    hexMap['B'] = "1011";
    hexMap['C'] = "1100";
    hexMap['D'] = "1101";
    hexMap['E'] = "1110";
    hexMap['F'] = "1111";
    string binary = "";
    for (int i = 0; i < SBox.size(); i++){
        binary += hexMap[SBox[i]];
    }
    return binary;
}

string shiftLeft(string str, int moves){
    string SBox = "";
    for (int i = 0; i < moves; ++i){
        for (int j = 1; j < 28; ++j){
            SBox += str[j];
        }
        SBox += str[0];
        str = SBox;
        SBox = "";
    }
    return str;
}

string Xor(string x1, string x2){
    string mid = "";
    for (int i = 0; i < x1.size(); ++i){
        if (x1[i] != x2[i]){
            mid += "1";
        }
        else{
            mid += "0";
        }
    } 
    return mid;
}

int main(){
    string key;
    
    fstream cipher_text;
    cipher_text.open("cipherText.txt", ios::in);

    string cipher;
    cipher_text >> cipher;
    key = "BABB09182736CCDD";
    // Key Generation

    // Hex to binary
    key = Hex_To_Bin(key);

    int key_comp[48] = {14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10,
        23, 19, 12, 4, 26, 8, 16, 7, 27, 20, 13, 2,
        41, 52, 31, 37, 47, 55, 30, 40, 51, 45, 33, 48,
        44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29, 32
    };

    // Parity bit drop table
    int keyp[56] = {57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18,
        10, 2, 59, 51, 43, 35, 27, 19, 11, 3, 60, 52, 44, 36,
        63, 55, 47, 39, 31, 23, 15, 7, 62, 54, 46, 38, 30, 22,
        14, 6, 61, 53, 45, 37, 29, 21, 13, 5, 28, 20, 12, 4
    };

    // getting 56 bit key from 64 bit using the parity bits
    key = permutation(key, keyp, 56); 

    // Splitting
    string left_per = key.substr(0, 28);
    string right_per = key.substr(28, 28);
    vector<string> round_key_array;  
    vector<string> round_key_binary; 

    for (int i = 0; i < 16; i++){
        right_per = shiftLeft(right_per, shift_table[i]);
        left_per = shiftLeft(left_per, shift_table[i]);

        // Combining
        string Combined = left_per + right_per;

        // Key Compression
        string RoundKey = permutation(Combined, key_comp, 48);

        round_key_array.push_back(Bin_To_Hex(RoundKey));
        round_key_binary.push_back(RoundKey);
    }

    cout << "\nDecryption Starts\n\n";
    reverse(round_key_binary.begin(), round_key_binary.end());
    reverse(round_key_array.begin(), round_key_array.end());
    string ptext = decryption(cipher, round_key_binary, round_key_array);
    cout << "\nPlain Text: " << ptext << endl;
    cout << "\nDecryption Ends\n";

    cipher_text.close();
    return 0;
}

string decryption(string plainText, vector<string> round_key_binary, vector<string> round_key_array){
    // Hexadecimal to binary
    plainText = Hex_To_Bin(plainText);

    // Initial Permutation Table
    int I_PERM[64] = {58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28,
                      20, 12, 4, 62, 54, 46, 38, 30, 22, 14, 6, 64, 56,
                      48, 40, 32, 24, 16, 8, 57, 49, 41, 33, 25, 17, 9,
                      1, 59, 51, 43, 35, 27, 19, 11, 3, 61, 53, 45, 37,
                      29, 21, 13, 5, 63, 55, 47, 39, 31, 23, 15, 7};
    // Initial Permutation
    plainText = permutation(plainText, I_PERM, 64);
    cout << "Initial permutation: " << Bin_To_Hex(plainText) << endl;

    // Splitting
    string left_per = plainText.substr(0, 32);
    string right_per = plainText.substr(32, 32);
    cout << "Splitting of initial permutation: L0=" << Bin_To_Hex(left_per) << " R0=" << Bin_To_Hex(right_per)
    << endl;

    // Expansion D-box Table
    int EXPANSION_D[48] = {32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9,
        8, 9, 10, 11, 12, 13, 12, 13, 14, 15, 16, 17,
        16, 17, 18, 19, 20, 21, 20, 21, 22, 23, 24, 25,
        24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1
    };

   
    cout << "\n";
    cout << "16 round decryption starts\n";
    cout << endl;
    for (int i = 0; i < 16; ++i){
        // Expansion D-box
        string right_expanded = permutation(right_per, EXPANSION_D, 48);

        string _xor = Xor(round_key_binary[i], right_expanded);

        // S-boxes
        string output = "";
        for (int i = 0; i < 8; i++){
            int row = int(_xor[i * 6 + 5] - '0') + 2 * int(_xor[i * 6] - '0') ;
            int col = 8 * int(_xor[i * 6 + 1] - '0') +
                      4 * int(_xor[i * 6 + 2] - '0') +
                      2 * int(_xor[i * 6 + 3] - '0') + int(_xor[i * 6 + 4] - '0');
            int val = SBox[i][row][col];
            output += char(val / 8 + '0');
            val = val % 8;
            output += char(val / 4 + '0');
            val = val % 4;
            output += char(val / 2 + '0');
            val = val % 2;
            output += char(val + '0');
        }
        // Straight D-box
        output = permutation(output, per, 32);

        // XOR left_per and output
        _xor = Xor(output, left_per);

        left_per = _xor;

        swap(left_per, right_per);
        cout << "Round-" << i + 1 << " " << Bin_To_Hex(left_per) << " "
             << Bin_To_Hex(right_per) << " " << round_key_array[i] << endl;
    }
    swap(left_per, right_per);
    // cout << "\nFinal Swapping : "
    //    << " " << Bin_To_Hex(left_per) << " " << Bin_To_Hex(right_per) << " " << endl;
    // Combination
    string Combined = left_per + right_per;

    // Final Permutation Table
    int final_perm[64] = {40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55,
        23, 63, 31, 38, 6, 46, 14, 54, 22, 62, 30, 37, 5,
        45, 13, 53, 21, 61, 29, 36, 4, 44, 12, 52, 20, 60,
        28, 35, 3, 43, 11, 51, 19, 59, 27, 34, 2, 42, 10,
        50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25
    };

    // Final Permutation
    string plaintext = Bin_To_Hex(permutation(Combined, final_perm, 64));
    return plaintext;
}



string permutation(string str, int *arr, int n)
{
    string per = "";
    for (int i = 0; i < n; i++)
    {
        per += str[arr[i] - 1];
    }
    return per;
}